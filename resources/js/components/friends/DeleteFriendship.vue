    ...
    <div class="form friendship_cancel">
        <div class="input_group">
            <h3 class="text-xl" v-if="friend.data">
                Do you want to delete friendship with "<b>{{friend.data.name}} {{friend.data.family}}</b>" ?
            </h3>
        </div>
        ...
        <div class="loading flex flex-col justify-center items-start h-16">
            ...
            <div v-else>
                <button class="btn cancel" @click="submitDelete()">Yes, Delete Friendship</button>
        ...
        methods: {
            async submitDelete(){
                ...
                axios({
                    ...
                    this.$parent.$emit('delete_value',this.friend);
                    this.loading = false;
                }).catch(error=>{
                    if(error.response){
